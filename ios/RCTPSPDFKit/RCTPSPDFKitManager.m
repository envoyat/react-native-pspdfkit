...
RCT_EXPORT_METHOD(saveDocumentWithPageIndex:(nonnull NSNumber *)reactTag 
  ...
  NSLog(@"[PSPDFKit] saveDocumentWithPageIndex called with tag: %@, pageIndex: %lu", reactTag, (unsigned long)pageIndex);
  dispatch_async(dispatch_get_main_queue(), ^{
    RCTPSPDFKitView *component = (RCTPSPDFKitView *)[self.bridge.uiManager viewForReactTag:reactTag];
    if ([component isKindOfClass:[RCTPSPDFKitView class]]) {
      NSLog(@"[PSPDFKit] Component found for tag: %@", reactTag);
      NSError *error;
      BOOL success;
      ...
      NSLog(@"[PSPDFKit] Component not found for tag: %@", reactTag);
